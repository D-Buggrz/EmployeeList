...
public class LoginServlet extends HttpServlet {
  ...
  protected void doGet(HttpServletRequest req, HttpServletResponse resp)
    ...
    if (req.getAttribute("loginDestination") != null) {
      ...
      req.getSession().setAttribute("loginDestination", "/locations");
      logger.log(Level.INFO, "logging destination /locations");
    }

    flow = new GoogleAuthorizationCodeFlow.Builder(
        HTTP_TRANSPORT,
        JSON_FACTORY,
        getServletContext().getInitParameter("location.clientID"),
        getServletContext().getInitParameter("location.clientSecret"),
        SCOPES)
        .build();
    ...
    String url =
        flow.newAuthorizationUrl()
            .setRedirectUri(getServletContext().getInitParameter("location.callback"))
