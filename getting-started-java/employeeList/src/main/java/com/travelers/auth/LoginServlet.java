...
public class LoginServlet extends HttpServlet {
  ...
  protected void doGet(HttpServletRequest req, HttpServletResponse resp)
    ...
    if (req.getAttribute("loginDestination") != null) {
      ...
      req.getSession().setAttribute("loginDestination", "/employees");
      logger.log(Level.INFO, "logging destination /employees");
    }

    flow = new GoogleAuthorizationCodeFlow.Builder(
        HTTP_TRANSPORT,
        JSON_FACTORY,
        getServletContext().getInitParameter("employee.clientID"),
        getServletContext().getInitParameter("employee.clientSecret"),
        SCOPES)
        .build();
    ...
    String url =
        flow.newAuthorizationUrl()
            .setRedirectUri(getServletContext().getInitParameter("employee.callback"))
